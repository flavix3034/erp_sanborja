...
<div class="row">
	<div class="col-sm-12">
		<div class="alert alert-<?= isset($rpta_msg) ? $rpta_msg : 'info' ?>">
			<?= $msg ?>
		</div>
	</div>
</div>
...
<form method="post" action="<?= base_url('servicios/save') ?>" id="form_servicio">
	<input type="hidden" name="modo" value="<?= isset($modo) ? $modo : 'insert' ?>">
	<?php if(isset($id)): ?>
	...
	<div class="panel panel-default">
		<div class="panel-heading">
			<h4><i class="glyphicon glyphicon-wrench"></i> Información del Servicio</h4>
		</div>
		<div class="panel-body">
			<div class="row filitas">
				<div class="col-sm-3">
			...
			<div class="row filitas">
				...
				<div class="col-sm-3">
					<label>Fecha Estimada de Reparaci&oacute;n *</label>
					<input type="datetime-local" name="fecha_estimada_reparacion" id="fecha_estimada_reparacion" class="form-control"
						   value="<?= isset($row->fecha_estimada_reparacion) && $row->fecha_estimada_reparacion ? date('Y-m-d\TH:i', strtotime($row->fecha_estimada_reparacion)) : '' ?>" required>
				</div>
				<div class="col-sm-3">
	...
	<div class="panel panel-default">
		...
		<div class="panel-body">
			<div class="row filitas">
				...
				<div class="col-sm-2">
					<label>&nbsp;</label><br>
					<button type="button" class="btn btn-success" onclick="agregarItemServicio()">
			...
			<div class="row">
				<div class="col-sm-12">
					<table class="table table-bordered table-condensed" id="tabla_items_servicio">
						<thead>
	...
	<div class="row filitas">
		<div class="col-sm-12">
			<button type="submit" class="btn btn-primary">
				<i class="glyphicon glyphicon-save"></i> Guardar Servicio
			</button>
			<a href="<?= base_url('servicios') ?>" class="btn btn-default">
				<i class="glyphicon glyphicon-arrow-left"></i> Cancelar
			</a>
			<?php if(isset($id)): ?>
			<a href="<?= base_url('servicios/view') ?>/<?= $id ?>" class="btn btn-info">
...
$(document).ready(function() {
    ...
    $('#form_servicio').submit(function() {
        ...
        if(fecha_estimada == '') {
            alert('La fecha estimada de reparacion es obligatoria');
            $('#fecha_estimada_reparacion').focus();
            return false;
        }
    ...
    $('#buscar_item').on('keyup', function() {
        ...
        busqueda_timeout = setTimeout(function(){
            $.get('<?= base_url("servicios/buscar_producto") ?>', {term: term}, function(data) {
                var html = '';
                if(data.length > 0) {
                    html = '<ul class="list-group dropdown-busqueda">';
                    ...
                    html = '<ul class="list-group dropdown-busqueda"><li class="list-group-item" style="padding:5px 10px; font-size:12px;">Sin resultados</li></ul>';
                }
    ...
    $(document).on('click', '.item-resultado', function() {
        ...
        var prod_serv = $(this).data('prod_serv');

        $('#item_id_selected').val(id);
        $('#item_name_selected').val(name);
        $('#item_impuesto_selected').val(impuesto);
        $('#item_prod_serv_selected').val(prod_serv);
        $('#buscar_item').val(name);
        $('#item_precio').val(price);
        $('#item_cantidad').val(1);
        $('#resultados_busqueda').hide().html('');
    });
...
function renderItemsServicio() {
    ...
    for(var i = 0; i < ar_serv_items.length; i++) {
        ...
        html += '<td>' + (item.obs || '') + '</td>';
        html += '<td class="text-center"><button type="button" class="btn btn-xs btn-danger" onclick="quitarItemServicio(' + i + ')"><i class="glyphicon glyphicon-trash"></i></button></td>';
        html += '</tr>';
...
#resultados_busqueda .list-group {
    margin: 0;
}
