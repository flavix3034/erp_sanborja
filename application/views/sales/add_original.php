		...
		<div class="col-lg-6 ventas">
			<form name="form1" id="form1" action="<?= base_url("sales/save") ?>" method="POST">
				...
				<div id="zona_cliente" style="display:none">
					...
					<div class="row">
						<div class="col-sm-12 col-lg-6 ventas">
							<!--<label><a href="<?= base_url('clientes/add/1'); ?>" target="_blank">Agregar Cliente:</a></label><br>-->
							<label>Nombres:</label>
							<input type="text" name="name_cliente" class="form-control" id="name_cliente">
				...
				<div class="row filitas">
					<div class="col-sm-10 col-md-3 col-lg-3 ventas">
						<label id="lbl_busqueda">Codigo de Barra</label><br>
						<input type="text" name="code" id="code" value="" class="form-control">
					</div>
					<div class="col-sm-10 col-md-8 col-lg-8">
						<label>&nbsp;</label><br>
						<select class="form-control" name="product_id" id="product_id" onblur="busqueda_precio();unexpand(this)" style="font-family:courier;font-weight: bold;">
						</select>
					</div>
					...
					<div class="col-sm-1">
						...
						<!--<input type="checkbox" name="chk_codigo" id="chk_codigo" onchange="cambios_chk_codigo(this)" checked>-->
						<div class="dropdown">
							<button type="button">...</button>
							<div class="dropdown-content" style="width:100px;left:-60px">
								<a href="#" onclick="$('#hdn_codigo').val('CODIGO');$('#lbl_busqueda').html('Codigo de Barra');">Codigo</a>
								<a href="#" onclick="$('#hdn_codigo').val('PRODUCTO');$('#lbl_busqueda').html('Producto');">Descripcion</a>
								<a href="#" onclick="$('#hdn_codigo').val('LIBRE');$('#lbl_busqueda').html('Producto Libre');">Crear Libre</a>
							</div>
						</div>
...
<script type="text/javascript">
	...
	/*function cambios_chk_codigo(obj){
		if($('#chk_codigo').is(':checked')){
			$("#lbl_busqueda").html('Codigo de Barra')
		}else{
			$("#lbl_busqueda").html('Busqueda Nombre')
		}
	}*/

	function mariposa_codigo(){
		console.log("En m..._codigo")
		$.ajax({
			data 	: {code : $("#code").val()},
			type 	: "POST",
            ...
            success: function(data){                                                    
				...
				$select.empty()
                for(registro in obj){
					valor 		= obj[registro]["id"]
					texto 		= obj[registro]["name"]
                }
	...
	$(document).ready(function(){
	    ...
	    $("#code").keyup(function(e){
	    	...
	    	if(document.getElementById('hdn_codigo').value == 'CODIGO'){ // por codigo de barra)	
		    	...
		    	if(e.key != 'ArrowDown'){
			        ...
			        if (consulta.length > 1){
				        ...
				        $.ajax({
				            ...
				            data: "b="+consulta,
				            //beforeSend: function(){ $("#code").html("<p align='center'><img src='ajax-loader.gif' /></p>"); },
				            error: function(){
				                alert("error petición ajax");
				            },
				            success: function(data){                                                    
								...
								$select.empty()
								for(registro in obj){
								  	//console.log( obj[registro]["code"] )	//console.log( obj[registro]["name"] )
								  	
									let valor = obj[registro]["id"]
									let texto = obj[registro]["name"]
									$select.append($("<option>", {
									    value: valor,
									    text: texto
									}));
				...
				// Descripcion libre
				//alert("Opcion inactiva.....")
			}

	    });
	    
	    $("#code").blur(function(){
	        var hdn_cod = $('#hdn_codigo').val()
	        if(hdn_cod == 'LIBRE'){
    	        const $select = $("#product_id");
                $select.empty()
                let valor = $("#code").val()
				...
				<div class="row filitas">	
					...
					<div class="col-sm-3 col-lg-2">
						<div class="row" style="margin-top:20px">
							...
							<button type="button" id="boton_mas" class="btn btn-success" onclick="Agregar()" style="font-size: 18px; font.font-weight: bold;">+</button>
							<!--<button type="button" onclick="llenar()">llenar</button>-->
						</div>
					</div>
					<div class="col-sm-4 col-md-3 col-lg-3 ventas text-center">
				...
				<div class="row filitas">
					<div class="col-sm-12" style="border-style:none; border-color:blue; border-width:1px">
						<!-- ESTE ES EL DIV DE LOS PRODUCTOS --->
						<div class="row">
		...
		<div class="col-lg-6 ventas" id="grid_productos">
	...
	<script type="text/javascript">
	    ...
	    function cargar_items(){
	        ...
	        cad += '<thead>'
	        cad += '    <tr class="active">'
	        cad += '        <th class="col-xs-5 col-sm-5">Producto</th>'
	        cad += '        <th class="col-xs-2 col-sm-2">Cant.</th>'
	        cad += '        <th class="col-xs-2 col-sm-2">Costo U.</th>'
	        cad += '        <th class="col-xs-2 col-sm-2" style="text-align:right">Item</th>'
			...
			for(let i=0; i<Limite; i++){
	            ...
	            // Descrip Producto
	            ctrl_descrip 	= '<input type="text" name="descripo[]" value="' + ar_items[i]["name"] + '" class="form-control">'
				
				// hidden id
				cad 			+= '<td style="text-align: left" class="col-xs-5 col-sm-5">' + ctrl_descrip + '<input type="hidden" name="item[]" value="'+ar_items[i]['id'] + '" class="form-control"></td>'
	            
	            // Cantidad
	            cad 			+= '<td class="col-xs-2 col-sm-2"><input size="4" style="text-align: right" type="text" name="quantity[]" value="' + ar_items[i]["quantity"] + '"  class="form-control" readonly></td>'
	            
	            // Costo
	            cad 			+= '<td class="col-xs-2 col-sm-2" style=""><input size="9" style="text-align: right;padding: 6.4px 6.4px" type="text" name="cost[]" value="' + ar_items[i]["cost_r"] + '"  class="form-control" readonly></td>'
				...
				// Subtotal
	            cad 			+= '<td class="col-xs-2 col-sm-2" style="padding-right:5px;"><input type="hidden" name="sub[]" value="' + nSubTotalx + '" style="text-align: right;" class="form-control" readonly>'
	            
	            // Un subtotal pero numerico
	            cad 			+=  '<input type="text" name="sss[]" value="' + n_sub_total + '" style="text-align: right;padding: 6.4px 6.4px;" class="form-control" readonly></td>'
	            
	            // Trash
	            cad 			+= '<td class="col-xs-2 col-sm-1" style="text-align: center"><a href="#" onclick="quitar_item(ar_items,'+i+')"><i class="fa fa-trash-o"></i></a></td>'
	        ...
	        // *** FILA SUBTOTAL **********
	        cad += '    <tr class="active">'
	        cad += '        <th class="col-xs-5 col-sm-5 celdas_totales"><?= lang('subtotal'); ?></th>'
	        cad += '        <th class="col-xs-2 col-sm-2 celdas_totales"></th>'
	        cad += '        <th class="col-xs-2 col-sm-2 celdas_totales"></th>'
	        cad += '        <th class="col-xs-2 col-sm-2 celdas_totales text-right"><span id="gsubtotal">0.00</span></th>'
	        cad += '        <th class="col-xs-2 col-sm-1 celdas_totales"></th>'
	        cad += '    </tr>'

	        // *** FILA IGV **********
	        cad += '    <tr class="active">'
	        cad += '        <th class="col-xs-5 col-sm-5 celdas_totales"><?= lang('igv'); ?></th>'
	        cad += '        <th class="col-xs-2 col-sm-2 celdas_totales"></th>'
	        cad += '        <th class="col-xs-2 col-sm-2 celdas_totales"></th>'
	        cad += '        <th class="col-xs-2 col-sm-2 celdas_totales text-right"><span id="gIgv">0.00</span></th>'
	        cad += '        <th class="col-xs-2 col-sm-1 celdas_totales"></th>'
	        cad += '    </tr>'
	        ...
	        if(nDscto > 0){
	            cad += '    <tr class="active">'
	            cad += '        <th style="font-weight:bold" class="col-xs-5 col-sm-5 celdas_totales">Dscto.</th>'
	            cad += '        <th class="col-xs-2 col-sm-2 celdas_totales"></th>'
	            cad += '        <th class="col-xs-2 col-sm-2 celdas_totales"></th>'
	            cad += '        <th class="col-xs-2 col-sm-2 celdas_totales text-right">-'+nDscto+'</th>'
	            cad += '        <th class="col-xs-2 col-sm-1 celdas_totales"></th>'
	            cad += '    </tr>'
	        }
	        ...
	        // *** FILA TOTAL **********
	        cad += '    <tr class="active">'
	        cad += '        <th class="col-xs-5 col-sm-5 celdas_totales">Total</th>'
	        cad += '        <th class="col-xs-2 col-sm-2 celdas_totales"></th>'
	        cad += '        <th class="col-xs-2 col-sm-2 celdas_totales"></th>'
	        cad += '        <th class="col-xs-2 col-sm-2 celdas_totales text-right"><span id="gTotal" style="font-size:16px">0.00</span></th>'
	        cad += '        <th class="col-xs-2 col-sm-1 celdas_totales"></th>'
	        cad += '    </tr>'
	        cad += '</tfoot>'
	        cad += "</table>"
