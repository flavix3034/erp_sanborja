...
class Servicios extends CI_Controller
{
    ...
    function getServicios(){
        ...
        $this->Servicios_model->getServicios($estado,$tecnico);
    }
    ...
    function cambiar_estado_servicio(){
        ...
        // Cambiar estado
        if($this->Servicios_model->cambiar_estado(
            $servicio_id, 
            $servicio->estado, 
            $estado_nuevo,
            $servicio->tecnico_asignado,
            $comentarios,
            $_SESSION["user_id"]
        )){
            $ar["rpta"] = "success";
            $ar["msg"] = "Estado cambiado correctamente";
        } else {
            $ar["rpta"] = "danger";
            $ar["msg"] = "No se pudo cambiar el estado";
        }
    ...
    function agregar_nota_servicio(){
        $servicio_id = $_POST['servicio_id'];
        $nota = $_POST['nota'];
        $tipo_nota = isset($_POST['tipo_nota']) ? $_POST['tipo_nota'] : 'GENERAL';
        
        // Obtener servicio para el técnico asignado
        $servicio = $this->Servicios_model->get_servicio_by_id($servicio_id);
        
        if($this->Servicios_model->agregar_nota(
            $servicio_id, 
            $nota, 
            $tipo_nota,
            $servicio ? $servicio->tecnico_asignado : null,
            $_SESSION["user_id"]
        )){
            $ar["rpta"] = "success";
            $ar["msg"] = "Nota agregada correctamente";
        } else {
            $ar["rpta"] = "danger";
            $ar["msg"] = "No se pudo agregar la nota";
        }
